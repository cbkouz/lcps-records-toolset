...
export class CalendarService {
  ...
  public getReportDateConfig(refDate: Date): ReportDateConfig {
    ...
    const safeRefDate = this.snapToWeekday(refDate);

    const refDateWeek = days.find(d => dateToString(d.date) === dateToString(safeRefDate))?.weekNumber;
  ...
  private snapToWeekday(date: Date): Date {
    const dayOfWeek = date.getDay();
    if (dayOfWeek > 0 && dayOfWeek < 6) return date; // Already a weekday
    const adjustedDate = new Date(date);
    const offset = dayOfWeek === 0 ? 2 : 1 // If Sunday, subtract 2 days to get to Friday; if Saturday, subtract 1 day
