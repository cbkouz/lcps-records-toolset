...
export class snowDaysRepository {
  ...
  public getUnprocessedSnowDays(): SnowDayQueue {
    ...
    const snowDayQueue: SnowDayQueue = values.reduce((queue, row, index) => {
      const rawDate = row[0];
      if (!rawDate || rawDate === "") {
        return queue; // Skip empty rows
      }
      const dateStr = dateToString(rawDate as Date);
  ...
  public markSnowDaysAsProcessed(rowsA1: string[]): void {
    rowsA1.forEach(a1 => {
      const cell = this.ss.getRange(a1);
      cell.setBackground(FORMATTING.COLORS.SNOW_DAY);
    });
  }
}
