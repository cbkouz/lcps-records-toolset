...
export class ReportRepository {
  ...
  public writeWeeklySummary(
    ...
    if (dateCols > 0) {
      sheet.setColumnWidths(2, dateCols, 45);
    }
    ...
    if (perfectAttendanceRanges.length > 0) {
      const perfectAttendanceRule = this.createPerfectAttendanceRule(perfectAttendanceRanges, dateCols);
  ...
  private createPerfectAttendanceRule(
    ...
    const dateEndColLetter = columnNumberToLetter((1 + dateCols) as SheetIndex);

    const formulaRange = `$${dateStartColLetter}${firstDataRow}:$${dateEndColLetter}${firstDataRow}`;
    const minimumPresent = Math.max(dateCols - 1, 1);

    const formula = `=AND(COUNTIF(${formulaRange}, "✓")>=${minimumPresent}, COUNTIF(${formulaRange}, "-")=0, COUNTIF(${formulaRange}, "S")=0)`;
