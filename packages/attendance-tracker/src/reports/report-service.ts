...
export class ReportService {
  ...
  public generateClassReports(classMap: ClassMap, logData: StudentHistoryMap): void {
    ...
    if (reportSheets.length === 0) {
      console.warn("No class report sheets found. Skipping class report generation.");
      return;
    }
  ...
  public generateWeeklyReport(classMap: ClassMap, dateConfig: ReportDateConfig, logData: StudentHistoryMap): void {
    ...
    if (dailyLookupKeys.length === 0) {
      throw new Error("No weekdays found for the selected report week.");
    }
    ...
    this.repo.writeWeeklySummary(matrix, formattingPlan, summaryCols, dailyLookupKeys.length);
  }
