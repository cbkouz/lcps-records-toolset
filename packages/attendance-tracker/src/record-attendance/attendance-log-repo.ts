...
export class AttendanceLogRepository {
  ...
  public readAttendanceLog(): { header: any[], data: any[][] } {
    if (this.logSheet.getLastRow() < 2) {
      return { header: [], data: [] }; // No records, return empty array
    }
    const [header, ...data] = this.logSheet.getDataRange().getValues();
    return { header, data };
  }

  public rewriteAttendanceLog(header: any[], data: any[][]): void {
    this.logSheet.clearContents();
    this.logSheet.getRange(1, 1, 1, header.length).setValues([header]);
    if (data.length > 0) {
      this.logSheet.getRange(2, 1, data.length, data[0].length).setValues(data);
    }
    console.log(`Rewrote attendance log with updated data.`);
  }     
}